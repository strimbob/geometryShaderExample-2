...
uniform float imgWidth, imgHeight,time;
float iGlobalTime;
...
vec4 computeEnlighting( in vec3 cameraPos, in vec3 cameraDir, in vec3 lightDir ) {
...
void main (void)  
{
 ...
 // vec2  iMouse =    lightDir.xy * gl_TexCoord[0].st;
    
    
    vec2  iMouse =    vec2(lightDirQ) *   gl_TexCoord[0].st;
    iGlobalTime = time;
     vec2 iResolution = vec2(100,100);
    //vec2 iMouse = vec2(lightDirQ);
    //camera
    ...
    vec2 q = gl_TexCoord[0].st /iResolution;
    vec2 p = -1.0 + 2.0*q;
	...
	vec4 enlighting = computeEnlighting( cameraPos, cameraDir, lightDir );

                        
	gl_FragColor = vec4(enlighting);
